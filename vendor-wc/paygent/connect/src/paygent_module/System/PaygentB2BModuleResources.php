...
use PaygentModule\Util\StringUtil;
 ...
 class PaygentB2BModuleResources {
	...
	function getUrl($telegramKind) {
		...
		// Set $prop array. Added by Shohei Tanaka 2022-01-26
		$hash_check = get_option( 'wc-paygent-hash_check' );
		if( $hash_check == '1'){
			$rs = str_replace('/n/','/s/',$rs);
		}
	...
	function readProperties() {
		...
		// Set property file. Changed by Shohei Tanaka 2022-01-26
		$test_mode = get_option('wc-paygent-testmode');
		if($test_mode == '1'){
			$properties_file = "/../sandbox_modenv_properties.php";
/*			if(get_option('wc-paygent-testmode') == 'test'){
				$clientFilePath = CLIENT_TEST_FILE_PATH;
			}else{
				$clientFilePath = CLIENT_TEST2_FILE_PATH;
			}*/
			$clientFilePath = CLIENT_TEST3_FILE_PATH;
			$defaultId = get_option('wc-paygent-test-cid');
			$defaultPassword = get_option('wc-paygent-test-cpass');
		}else{
			$properties_file = "/../modenv_properties.php";
			$clientFilePath = CLIENT_FILE_PATH;
			$defaultId = get_option('wc-paygent-cid');
			$defaultPassword = get_option('wc-paygent-cpass');
		}
		// end here...

		$prop = PaygentB2BModuleResources::parseJavaProperty(dirname(__FILE__) . $properties_file);//remove
		if ($prop === false) {
		...
		$hash_check = get_option( 'wc-paygent-hash_check' );
		if( $hash_check == '1'){
			$prop[$this->app['const']['PaygentB2BModuleResources__NOT_USE_CLIENT_CERT']] = 'true';
			$prop[$this->app['const']['PaygentB2BModuleResources__NOT_USE_CA_CERT']] = 'true';
			$prop[$this->app['const']['PaygentB2BModuleResources__CLIENT_FILE_PATH']] = null;
			$prop[$this->app['const']['PaygentB2BModuleResources__CA_FILE_PATH']] = null;
		}
		// end here...
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__CLIENT_FILE_PATH'], $prop)
			...
			$this->clientFilePath = $clientFilePath;//by Shohei Tanaka
		}

        // クライアント証明書未使用設定 2022-01-26 by Shohei Tanaka
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__CA_FILE_PATH'], $prop)
			...
			$this->caFilePath = CA_FILE_PATH;//by Shohei Tanaka
		}

        // CA証明書未使用設定 2022-01-26 Shohei Tanaka
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__DEFAULT_ID'], $prop)
			...
			$this->defaultId = $defaultId;//remove
		}
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__DEFAULT_PASSWORD'], $prop)
			...
			$this->defaultPassword = $defaultPassword;//remove
		}
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__LOG_OUTPUT_PATH'], $prop)
			...
			$this->logOutputPath = WC_LOG_DIR.'wc-paygent-log.log';//remove
		}
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__DEBUG_FLG'], $prop)
			...
			if(get_option('wc-paygent-debug') == '1'){// Change by Shohei Tanaka
				$this->debugFlg = 1;
			}else{
